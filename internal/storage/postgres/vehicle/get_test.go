...
func TestVehicleRepository_GetByWateringPlanID(t *testing.T) {
	ctx := context.Background()
	suite.ResetDB(t)
	suite.InsertSeed(t, "internal/storage/postgres/seed/test/vehicle")
	...
	t.Run("should return vehicle with type transporter by watering plan id", func(t *testing.T) {
		...
		// when
		got, err := r.GetByWateringPlan(ctx, int32(1), entities.VehicleTypeTransporter)
	...
	t.Run("should return vehicle with type trailer by watering plan id", func(t *testing.T) {
		...
		// when
		got, err := r.GetByWateringPlan(ctx, int32(1), entities.VehicleTypeTrailer)
	...
	t.Run("should return error when watering plan not found", func(t *testing.T) {
		...
		// when
		got, err := r.GetByWateringPlan(ctx, int32(99), entities.VehicleTypeTrailer)

		// then
		assert.Error(t, err)
		assert.Nil(t, got)
		assert.Equal(t, "watering plan not found", err.Error())
	})
	...
	t.Run("should return error when vehicle with type trailer is not found", func(t *testing.T) {
		...
		// when
		got, err := r.GetByWateringPlan(ctx, int32(2), entities.VehicleTypeTrailer)
	...
	t.Run("should return error when watering plan id is zero", func(t *testing.T) {
		...
		// when
		got, err := r.GetByWateringPlan(ctx, int32(0), entities.VehicleTypeTransporter)
	...
	t.Run("should return error when vehicle type is not trailer or transporter", func(t *testing.T) {
		...
		// when
		got, err := r.GetByWateringPlan(ctx, int32(1), entities.VehicleTypeUnknown)
