...
ALTER SEQUENCE sensors_id_seq RESTART WITH 4;

INSERT INTO trees (id, tree_cluster_id, sensor_id, age, height_above_sea_level, planting_year, species, tree_number, latitude, longitude, geometry)
VALUES 
  (1, 1, 1, 3, 10.0, 2021, 'Quercus robur', 1, 54.82124518093376, 9.485702120628517, ST_SetSRID(ST_MakePoint(54.82124518093376, 9.485702120628517), 4326)),
  (2, 1, NULL, 2, 11.0, 2022, 'Quercus robur', 1, 54.8215076622281, 9.487153277881877, ST_SetSRID(ST_MakePoint(54.8215076622281, 9.487153277881877), 4326)),
  (3, 1, NULL, 1, 13.3, 2023, 'Quercus robur', 1, 54.82078826498143, 9.489684366114483, ST_SetSRID(ST_MakePoint(54.82078826498143, 9.489684366114483), 4326)),
  (4, 2, 2, 4, 10.0, 2020, 'Quercus robur', 1, 54.78780993841013, 9.444052105200551, ST_SetSRID(ST_MakePoint(54.78780993841013, 9.444052105200551), 4326)),
  (5, 2, NULL, 3, 11.0, 2021, 'Quercus robur', 1, 54.78836553796373, 9.444075995492044, ST_SetSRID(ST_MakePoint(54.78836553796373, 9.444075995492044), 4326)),
  (6, 2, NULL, 2, 13.3, 2022, 'Quercus robur', 1, 54.787768612518455, 9.443996361187065, ST_SetSRID(ST_MakePoint(54.787768612518455, 9.443996361187065), 4326));
ALTER SEQUENCE trees_id_seq RESTART WITH 7;

INSERT INTO sensor_data (id, sensor_id, data)
VALUES 
  (1, 1, '{"temperature": 20.0, "humidity": 0.5, "moisture": 0.75}'),
  (2, 1, '{"temperature": 21.0, "humidity": 0.6, "moisture": 0.5}'),
  (3, 1, '{"temperature": 22.0, "humidity": 0.7, "moisture": 0.25}'),
  (4, 2, '{"temperature": 20.0, "humidity": 0.5, "moisture": 0.75}'),
  (5, 2, '{"temperature": 21.0, "humidity": 0.6, "moisture": 0.5}'),
  (6, 2, '{"temperature": 22.0, "humidity": 0.7, "moisture": 0.25}'),
  (7, 3, '{"temperature": 20.0, "humidity": 0.5, "moisture": 0.75}'),
  (8, 3, '{"temperature": 21.0, "humidity": 0.6, "moisture": 0.5}'),
  (9, 3, '{"temperature": 22.0, "humidity": 0.7, "moisture": 0.25}');
...
ALTER SEQUENCE user_vehicles_id_seq RESTART WITH 1;
...
ALTER SEQUENCE trees_id_seq RESTART WITH 1;

DELETE FROM tree_clusters WHERE id = 1;
DELETE FROM tree_clusters WHERE id = 2;
ALTER SEQUENCE tree_clusters_id_seq RESTART WITH 1;
