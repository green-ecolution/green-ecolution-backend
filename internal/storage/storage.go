...
type WateringPlanRepository interface {
	...
	// GetAll returns all watering plans with same watering plan status
	GetAllByStatus(ctx context.Context, status entities.WateringPlanStatus) ([]*entities.WateringPlan, error)
