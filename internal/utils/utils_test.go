...
func TestScheduler(t *testing.T) {
	t.Run("should call process function at least once", func(t *testing.T) {
		...
		var callCount int
		process := func(ctx context.Context) string {
			callCount++
			return "called"
		}
	...
	t.Run("should stop calling process if context is canceled", func(t *testing.T) {
		...
		var callCount int
		process := func(ctx context.Context) int {
			callCount++
			return callCount
		}
	...
	t.Run("should handle process function returning int", func(t *testing.T) {
		// given
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()

		var called bool
		processInt := func(ctx context.Context) int {
			called = true
			return 42
		}
