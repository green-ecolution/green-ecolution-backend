/*
Green Space Management API

This is the API for the Green Ecolution Management System.

API version: develop
Contact: info@green-ecolution.de
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the VehicleList type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &VehicleList{}

// VehicleList struct for VehicleList
type VehicleList struct {
	Data []Vehicle `json:"data"`
	Pagination Pagination `json:"pagination"`
}

type _VehicleList VehicleList

// NewVehicleList instantiates a new VehicleList object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewVehicleList(data []Vehicle, pagination Pagination) *VehicleList {
	this := VehicleList{}
	this.Data = data
	this.Pagination = pagination
	return &this
}

// NewVehicleListWithDefaults instantiates a new VehicleList object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewVehicleListWithDefaults() *VehicleList {
	this := VehicleList{}
	return &this
}

// GetData returns the Data field value
func (o *VehicleList) GetData() []Vehicle {
	if o == nil {
		var ret []Vehicle
		return ret
	}

	return o.Data
}

// GetDataOk returns a tuple with the Data field value
// and a boolean to check if the value has been set.
func (o *VehicleList) GetDataOk() ([]Vehicle, bool) {
	if o == nil {
		return nil, false
	}
	return o.Data, true
}

// SetData sets field value
func (o *VehicleList) SetData(v []Vehicle) {
	o.Data = v
}

// GetPagination returns the Pagination field value
func (o *VehicleList) GetPagination() Pagination {
	if o == nil {
		var ret Pagination
		return ret
	}

	return o.Pagination
}

// GetPaginationOk returns a tuple with the Pagination field value
// and a boolean to check if the value has been set.
func (o *VehicleList) GetPaginationOk() (*Pagination, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Pagination, true
}

// SetPagination sets field value
func (o *VehicleList) SetPagination(v Pagination) {
	o.Pagination = v
}

func (o VehicleList) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o VehicleList) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["data"] = o.Data
	toSerialize["pagination"] = o.Pagination
	return toSerialize, nil
}

func (o *VehicleList) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"data",
		"pagination",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varVehicleList := _VehicleList{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varVehicleList)

	if err != nil {
		return err
	}

	*o = VehicleList(varVehicleList)

	return err
}

type NullableVehicleList struct {
	value *VehicleList
	isSet bool
}

func (v NullableVehicleList) Get() *VehicleList {
	return v.value
}

func (v *NullableVehicleList) Set(val *VehicleList) {
	v.value = val
	v.isSet = true
}

func (v NullableVehicleList) IsSet() bool {
	return v.isSet
}

func (v *NullableVehicleList) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableVehicleList(val *VehicleList) *NullableVehicleList {
	return &NullableVehicleList{value: val, isSet: true}
}

func (v NullableVehicleList) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableVehicleList) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


